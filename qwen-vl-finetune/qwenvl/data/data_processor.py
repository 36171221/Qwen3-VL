...
from .nav_utils import (
    build_token_node_ids,
    expand_distance_matrix_with_node_ids,
    resolve_nav_image,
    load_npz_cached,
)
        ...
        if role == "user":
            ...
            for seg in text_parts:
                if seg == "<image>":
                    ...
                    image_spec = image_pool.pop(0)
                    if isinstance(image_spec, dict) and "image" in image_spec:
                        content.append(
                            {
                                "type": "image",
                                "image": resolve_nav_image(
                                    image_spec["image"],
                                    base_path,
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        source = sources[0]
        graph_sprels = None
        nav_pair_dist_path = source.get("nav_pair_dist_path")
        if nav_pair_dist_path and source.get("id") is not None:
            try:
                nav_pair_dists = load_npz_cached(nav_pair_dist_path)[str(source["id"])]
            except KeyError:
                nav_pair_dists = None
        else:
            nav_pair_dists = None
        ...
        graph_sprels = expand_distance_matrix_with_node_ids(
            nav_pair_dists,
            token_node_ids,
...
def pad_square_matrices(matrix_list, padding_side="right", padding_value=0.0):
    max_length = max(matrix.shape[0] for matrix in matrix_list)
    padded_matrices = []
...
class DataCollatorForSupervisedDataset(object):
    ...
    include_graph_sprels: bool = False
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        original_lengths = [ids.shape[0] for ids in input_ids]
        graph_sprels = []
        for instance, cur_len in zip(instances, original_lengths):
            graph_sprel = instance.get("graph_sprels")
            if graph_sprel is None:
                graph_sprel = torch.zeros((cur_len, cur_len), dtype=torch.float32)
            else:
                graph_sprel = graph_sprel[:cur_len, :cur_len]
            graph_sprels.append(graph_sprel)
        ...
        graph_sprels = [
            graph_sprel[: self.tokenizer.model_max_length, : self.tokenizer.model_max_length]
            for graph_sprel in graph_sprels
        ]
        batch = dict(
            input_ids=input_ids,
            labels=labels,
        ...
        if self.include_graph_sprels:
            batch["graph_sprels"] = pad_square_matrices(
                graph_sprels,
...
def make_supervised_data_module(processor, data_args) -> Dict:
    ...
    if data_args.data_flatten or data_args.data_packing:
        data_collator = FlattenedDataCollatorForSupervisedDataset(
            processor.tokenizer, include_graph_sprels=data_args.use_nav_graph
        )
        return dict(
